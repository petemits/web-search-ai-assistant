...
The project contains a configuration file (`.env, .env.example`).
- Open the file and fill in any required credentials (e.g., `username`, `password`, `API_KEY`).
- For `.env` files, create a copy from `.env.example` if provided.
### 5️⃣ Run the Program
```bash
npm start
```
